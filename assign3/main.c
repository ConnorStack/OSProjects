...
void *cpu_scheduler_thread(void *args)
{
    ...
    while (1)
    {
        ...
        else if (strcmp(scheduler_alg, "PR") == 0)
        {
            ...
            PCB *delistingPCB = delist_specific_pcb(ready_queue, pcb);
            pthread_mutex_unlock(&ready_queue_mutex);

            // print_PCB(delistingPCB);
            printf("\n");
            print_PCB(pcb);
            printf("\n");
            if (pcb != NULL)
